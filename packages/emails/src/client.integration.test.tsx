...
describe("Status Report Email - Unsubscribe Link in Body", () => {
  ...
  test("should include unsubscribe link in email body when URL is provided", async () => {
    ...
    expect(html).toContain("Du bekommst diese E-Mail");
  });
...
describe("Status Report Email - Subject Line", () => {
  ...
  test('prepends "Behoben:" only when status is "resolved"', () => {
  ...
  test("uses the bare report title for ongoing report statuses", () => {
    ...
    for (const status of ongoing) {
      ...
      expect(subject).not.toContain("Behoben:");
    }
  });

  test('prepends "Geplante Wartung:" when status is "maintenance"', () => {
    expect(statusReportSubject({ status: "maintenance", reportTitle })).toBe(
      `Geplante Wartung: ${reportTitle}`,
    );
  });
});
