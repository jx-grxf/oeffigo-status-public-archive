...
import { Footer, IMPRINT_URL } from "../emails/_components/footer";
...
describe("primitives", () => {
  ...
  test("layout sets the preheader, hosted logo and light colour scheme", async () => {
    ...
    expect(html).toContain("https://status.oeffigo.app/brand/mark.png");
...
describe("status report", () => {
  test("eyebrow, components, markdown, links", async () => {
    ...
    expect(html).toContain("WIRD BEOBACHTET");
    expect(html).toContain("Update 3 · 18.09.2026, 14:37 · seit 2h 14m");
    expect(html).toContain("API unavailable");
    expect(html).toContain("Runners");
    expect(html).toContain("Teilweise gestört");
    expect(html).toContain("Funktioniert");
    expect(html).not.toContain("●");
  ...
  test("pill names each status in German, one tone per status", async () => {
    const expected = {
      investigating: [tones.danger, "WIRD UNTERSUCHT"],
      identified: [tones.warning, "URSACHE GEFUNDEN"],
      monitoring: [tones.info, "WIRD BEOBACHTET"],
      resolved: [tones.success, "BEHOBEN"],
      maintenance: [tones.info, "WARTUNG"],
    } as const;
  ...
  test("unknown impact drops the value cell", async () => {
    ...
    expect(html).toContain("Runners");
    expect(html).toContain('colSpan="2"');
    expect(html).not.toContain("Funktioniert");
    expect(html).not.toContain("Teilweise gestört");
  });
  ...
  test("optional blocks disappear", async () => {
    ...
    expect(html).not.toContain("seit 2h 14m");
    expect(html).not.toContain("Auf der Statusseite verfolgen");
    expect(html).not.toContain("●");
  });
  ...
  test("preheader never repeats the subject", () => {
    expect(statusReportPreheader(report)).toBe(
      "Wird beobachtet: API, Runners.",
    );
...
describe("every transactional template", () => {
  ...
  for (const [name, element] of Object.entries(all)) {
    test(`${name}: brand casing, no SVG data URIs, no flex/grid, 600px`, async () => {
      ...
      expect(html).toContain(IMPRINT_URL);
    });
  }
});
