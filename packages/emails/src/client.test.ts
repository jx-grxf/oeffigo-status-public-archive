...
describe("EmailClient self-host delivery", () => {
  let client: EmailClient;
  let previousSender: string | undefined;
  beforeEach(() => {
    previousSender = process.env.EMAIL_FROM;
    process.env.EMAIL_FROM = "ÖffiGo <status@example.com>";
    client = new EmailClient({ apiKey: "re_test", retryBackoff: "0 millis" });
  });
  afterEach(() => {
    if (previousSender === undefined) delete process.env.EMAIL_FROM;
    else process.env.EMAIL_FROM = previousSender;
  });
  test("sends confirmations with the configured sender", async () => {
    const send = stub(client.client.emails, "send", () => Promise.resolve(ok));
    try {
      await client.sendPageSubscription({
        to: "subscriber@example.com",
        page: "ÖffiGo",
        link: "https://status.example.com/verify/test",
      });
      expect(send.calls[0].args[0].from).toBe("ÖffiGo <status@example.com>");
    } finally {
      send.restore();
    }
  });
  test("propagates rejected confirmation and magic-link sends without provider details", async () => {
    const send = stub(client.client.emails, "send", () =>
      Promise.resolve({
        ...fail,
        error: {
          name: "validation_error",
          message: "secret subscriber@example.com",
        },
      }),
    );
    try {
      const request = {
        to: "subscriber@example.com",
        page: "ÖffiGo",
        link: "https://status.example.com/verify/secret",
      };
      await expect(client.sendPageSubscription(request)).rejects.toThrow(
        "Email delivery failed",
      );
      await expect(client.sendStatusPageMagicLink(request)).rejects.toThrow(
        "Email delivery failed",
      );
    } finally {
      send.restore();
...
describe("EmailClient.sendMonitorAlert", () => {
  ...
  test("rethrows a Resend error without provider details so the outbox can retry", async () => {
    send.restore();
    send = stub(client.client.emails, "send", () => Promise.resolve(fail));

    await expect(client.sendMonitorAlert(req)).rejects.toThrow(
      "Email delivery failed",
    );
  });
});
