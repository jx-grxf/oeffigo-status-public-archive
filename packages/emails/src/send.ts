...
import { env } from "./env";
import { resolveEmailFrom, resolveEmailReplyTo } from "./sender";
...
// A self-hosted sender must not reply to or send as the upstream vendor.
function senderFields(from: string, replyTo?: string) {
  const configuredReplyTo = resolveEmailReplyTo();
  const fallbackReplyTo =
    process.env.SELF_HOST === "true" ? undefined : replyTo;
  const resolvedReplyTo = configuredReplyTo ?? fallbackReplyTo;
  return {
...
export const sendEmail = async (
  ...
  const { data, error } = await resend.emails.send(
    {
      ...email,
      ...senderFields(email.from, reply_to),
