...
import {
  claimMailCooldown,
  MailCooldownError,
} from "../../security/mail-cooldown";
import { verifyTurnstile, TurnstileError } from "../../security/turnstile";
import {
  createTRPCRouter,
  protectedProcedure,
  publicProcedure,
    ...
    .mutation(async (opts) => {
      try {
        await verifyTurnstile({
          token: opts.input.turnstileToken,
          action: "status-subscribe",
          requestHostname: opts.ctx.req
            ? new URL(opts.ctx.req.url).hostname
            : undefined,
        });
      } catch (error) {
        throw new TRPCError({
      ...
      if (
        ...
        subscriber.expiresAt.getTime() <= Date.now() ||
      ...
      try {
        await claimMailCooldown("subscribe", subscriber.email);
      } catch (error) {
