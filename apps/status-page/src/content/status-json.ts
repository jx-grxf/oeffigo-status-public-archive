...
export function toStatus(page: Page, baseUrl: string, live?: MergedStatus) {
  return {
    page: pageBlock(page, baseUrl),
    status: live ? serviceIndicator(live.overall) : pageIndicator(page.status),
  };
}
...
export function toSummary(
  ...
  const components = live
    ? live.services.map((s) => ({
        name: s.en,
        status: serviceComponentStatus(s.state),
      }))
    : flattenComponents(page.trackers).map((c) => ({
        name: c.name,
