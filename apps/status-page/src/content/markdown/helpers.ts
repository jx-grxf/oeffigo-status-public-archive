...
/**
 * Pointer to the machine-readable formats. Public pages only — a gated page
 * must not advertise JSON endpoints that may not honor its access gate.
 */
export function machineReadable(): string {
  return "Machine-readable: [current.json](/api/status/current.json) · [summary.json](/api/status/summary.json)";
}
