...
export async function GET(
  ...
  try {
    ...
    const live =
      process.env.OEFFIGO_SINGLE_TENANT === "true" && data.slug === "oeffigo"
        ? await readCombinedStatus(data.trackers)
        : undefined;
    const responsePayload =
      endpoint === "status"
        ? toStatus(data, baseUrl, live)
        : endpoint === "incidents"
          ? toUnresolvedIncidents(data, baseUrl)
          : toSummary(data, baseUrl, Date.now(), live);

    const body = JSON.stringify(responsePayload);
    const etag = computeETag(body);
    const cacheControl =
      !live && data.accessType === "public"
