...
function markdownResponse(
  ...
  const response = new NextResponse(finalBody, { status, headers });
  if (process.env.OEFFIGO_SINGLE_TENANT === "true")
  ...
  try {
    ...
    switch (target.kind) {
      ...
      case "events": {
        ...
        if (
          process.env.OEFFIGO_SINGLE_TENANT === "true" &&
          page.slug === "oeffigo"
        ) {
          page = withCombinedStatus(
            page,
