...
const {
  ...
  adapter,
  logger,
  // This panel publishes to a public status page, so a forgotten browser
  // stays signed in for a week rather than a month.
  session: { maxAge: 7 * 24 * 60 * 60, updateAge: 24 * 60 * 60 },
  providers: [
    ...(isGitHubLoginConfigured() ? [GitHubProvider] : []),
    ...(process.env.SELF_HOST !== "true" &&
    process.env.AUTH_GOOGLE_ID &&
    process.env.AUTH_GOOGLE_SECRET
      ? [GoogleProvider]
      : []),
    ...(process.env.SELF_HOST !== "true" && process.env.AUTH_OIDC_ISSUER
      ? [OIDCProvider]
      : []),
    ...(process.env.SELF_HOST !== "true" && hasWorkOS ? [WorkOSProvider] : []),
  ...
  callbacks: {
    ...
    async signIn(params) {
      if (params.account?.provider === "github") {
        if (
          !params.profile ||
          !isAllowedGitHubOwner(params.profile.id) ||
          params.account.providerAccountId !== String(params.profile.id) ||
          !isAllowedOwner(params.user.email)
        )
          return false;
        const profile = mapGitHubOwnerProfile(params.profile);
      ...
      if (
        process.env.SELF_HOST === "true" &&
        (params.account?.provider !== "resend" ||
          !isAllowedOwner(params.user.email))
      ) {
        return false;
      }
  ...
  events: {
    ...
    async createUser(params) {
      if (process.env.SELF_HOST === "true") return;
